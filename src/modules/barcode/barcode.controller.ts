...
class Controller extends BaseController {
  ...
  processPreOrderBarcodes = this.catchAsync(
    async (req: Request, res: Response) => {
      const { order_id: orderId } = req.params;
      const { barcodes } = req.body;
      ...
      const result = await UniqueBarcodeService.processPreOrderBarcodes(
        orderId,
        barcodes,
        updatedBy
      );
  ...
  processReturnBarcodesForPreOrder = this.catchAsync(
    async (req: Request, res: Response) => {
      const { order_id: orderId } = req.params;
      const { barcodes } = req.body;
      ...
      const result =
        await UniqueBarcodeService.processReturnBarcodesForPreOrder(
          orderId,
          barcodes,
          updatedBy
        );
