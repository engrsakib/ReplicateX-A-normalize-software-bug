...
class Service {
  ...
  async processPreOrderBarcodes(
    ...
    try {
      ...
      // ✅ [1. NEW CODE START] খরচের হিসাব রাখার ভ্যারিয়েবল
      let totalTransactionCost = 0;
      // ✅ [1. NEW CODE END]
      ...
      for (const [, group] of Array.from(groups.entries())) {
        ...
        if (!orderItem) {
          throw new ApiError(
            HttpStatusCode.BAD_REQUEST,
            `The order doesn’t include any item with the specified barcode ${group.barcodes.join(
              ", "
            )}`
          );
        }
        ...
        if (alreadyAssigned + qty > orderItem.quantity) {
          throw new ApiError(
            HttpStatusCode.BAD_REQUEST,
            `Too many barcodes for item ${group.product}.`
          );
        }
        ...
        for (const doc of group.docs) {
          ...
          const updateLog = {
            ...
            role: updatedBy.role,
            date: updatedBy.date,
            system_message: `Assigned to PreOrder #${
              order.order_id
            } on ${new Date().toLocaleString("en-GB", {
              timeZone: "Asia/Dhaka",
            })}; Prev status: ${prevStatus}; Prev conditions: ${prevConditions}`,
          };
        ...
        // D. Aggregate stock and lot counts
        const stockMap = new Map<string, number>();
        const lotMap = new Map<string, number>();
        for (const d of claimedDocs) {
          if (d.stock) {
        ...
        for (const [stockId, count] of Array.from(stockMap.entries())) {
          ...
          if (!updatedStock) {
            throw new ApiError(
              HttpStatusCode.BAD_REQUEST,
              `Insufficient stock for stockId ${stockId}`
            );
          }
        }

        // F. Decrease Lot & Calculate Cost
        for (const [lotId, count] of Array.from(lotMap.entries())) {
          const updatedLot = await LotModel.findOneAndUpdate(
            { _id: lotId, qty_available: { $gte: count } },
            { $inc: { qty_available: -count } },
          ...
          // ✅ [2. NEW CODE START] লটের খরচ হিসাব করা
          const unitCost = updatedLot.cost_per_unit || 0;
          const totalCostForThisLot = unitCost * count;
          totalTransactionCost += totalCostForThisLot;
          // ✅ [2. NEW CODE END]
      ...
      // ✅ [3. NEW CODE START] অর্ডারে খরচ আপডেট করা
      order.sys_ref_value = (order.sys_ref_value || 0) + totalTransactionCost;
      // ✅ [3. NEW CODE END]
  ...
  async processReturnBarcodesForPreOrder(
    ...
    try {
      ...
      // 1. Fetch Order (PreOrderModel থেকে)
      const order = await PreOrderModel.findOne({ order_id: orderId }).session(
        session
      );
      ...
      for (const [, group] of Array.from(groups.entries())) {
        ...
        // ✅ [A] Refund Value Calculation
        if (orderItem.price) {
          totalRefundValue += orderItem.price * qty;
        }

        // [B] Barcode Claim (স্ট্যাটাস IN_STOCK করা)
        // নোট: আগের কোডে REFURBISHING ছিল, কিন্তু স্ট্যান্ডার্ড করতে IN_STOCK দিলাম।
        // আপনি চাইলে REFURBISHING রাখতে পারেন, তবে Cost Accounting এর জন্য IN_STOCK লজিকই বেটার।
        const claimedDocs: Array<(typeof barcodeDocs)[number]> = [];
        for (const doc of group.docs) {
          const updateLog = {
            name: updatedBy.name,
            role: updatedBy.role,
            date: updatedBy.date,
            system_message: `Returned PreOrder #${order.order_id} on ${new Date().toLocaleString("en-GB", { timeZone: "Asia/Dhaka" })}`,
          };
          ...
          const claimed = await BarcodeModel.findOneAndUpdate(
            ...
            {
              $set: {
                status: productBarcodeStatus.IN_STOCK, // Standardized to IN_STOCK
                is_used_barcode: true,
              },
              $push: { updated_logs: { $each: [updateLog], $position: 0 } },
            },
      ...
      return {
        success: true,
        message: "PreOrder Barcodes assigned & Amount adjusted successfully",
      };
    } catch (error) {
      await session.abortTransaction();
      throw error;
